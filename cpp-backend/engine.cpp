...
// High-Performance C++ Database Connector & Storage Engine
// Features: Role-based Directory Access (Sole Admin vs Staff), Staff Status Portal,
// SQLite WAL mode, sub-millisecond query execution, and JSON serialisation.

namespace ProtonCore {
...
public:
    ...
    bool execute(const std::string& sql) {
        ...
        if (rc != SQLITE_OK) {
            if (err) {
                std::cerr << "SQL error: " << err << " in: " << sql << std::endl;
                sqlite3_free(err);
            }
            return false;
        }
    ...
    void seedInitialDataIfEmpty() {
        ...
        // 1. Sole System Administrator (Screenshot 2: "— | System Administrator | kanithisaikiran3036@gmail.com | Admin | Active")
        execute(R"(
        ...
        // 2. Staff 1: Sarah Jenkins (Screenshot 2: "STF-1001 | Sarah Jenkins | s.jenkins@nexus.corp | Staff | Active")
        execute(R"(
        ...
        // 3. Staff 2: Alex Rivera (Screenshot 2: "STF-1002 | Alex Rivera | a.rivera@nexus.corp | Staff | Active")
        execute(R"(
            INSERT INTO users (id, staff_id, name, email, password, role, status, department, location, phone, created_at, updated_at)
            VALUES (
                'u_staff_1002',
        ...
        execute(R"(
            ...
            ('act_3', 'u_admin_sole', 'System Administrator', 'System Initialized', 'Sole administrator provisioned directory', datetime('now', '-30 days'));
        )");
    ...
    // List users (Matching Screenshot 2 exactly: STF-1002, STF-1001, Admin)
    std::string listUsers(const std::string& search = "") {
        ...
        // Ordered such that STF-1002 comes first, STF-1001 second, Admin third matching Screenshot 2!
    ...
    // Get specific user status / profile
    ...
    // Generate next Staff ID (e.g. STF-1003)
    ...
    // Update user - Admin can edit all; Staff can only edit their own profile
    std::string updateUser(const std::string& callerId, const std::string& targetUserId, const std::string& jsonInput) {
        ...
        if (isAdmin) {
            ...
            // Staff can only update their phone/password
        ...
        if (!password.empty()) {
            ...
            if (sqlite3_prepare_v2(db, "UPDATE users SET password=? WHERE id=?;", -1, &pwdStmt, nullptr) == SQLITE_OK) {
        ...
        logActivity(targetUserId, isAdmin ? "System Administrator" : name, "Profile Updated", "User record updated");
        return "{\"success\":true,\"user\":" + getUserStatus(targetUserId) + "}";
    }

    // Delete user - ONLY ADMIN CAN DELETE, cannot delete sole admin!
    std::string deleteUser(const std::string& callerId, const std::string& targetUserId) {
        ...
        if (targetUserId == "u_admin_sole") {
            return "{\"success\":false,\"error\":\"Cannot delete the sole system administrator account.\"}";
        }

        sqlite3_stmt* stmt = nullptr;
        std::string sql = "DELETE FROM users WHERE id = ?;";
        if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
            return "{\"success\":false,\"error\":\"Delete prepare error\"}";
        }
        sqlite3_bind_text(stmt, 1, targetUserId.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);

        logActivity(targetUserId, "System Administrator", "User Deleted", "Staff record deleted");
        return "{\"success\":true}";
    }

    // System Overview matching Screenshot 1:
    // Total Users (3), Active Users (3), Staff (2), Departments (5), Locations (5)
    std::string getDashboardOverview() {
        int totalUsers = 0;
        int activeUsers = 0;
        int staffCount = 0;
        int departmentsCount = 5;
        int locationsCount = 5;

        sqlite3_stmt* stmt = nullptr;
        if (sqlite3_prepare_v2(db, "SELECT COUNT(*), SUM(CASE WHEN status = 'Active' THEN 1 ELSE 0 END), SUM(CASE WHEN role = 'Staff' THEN 1 ELSE 0 END) FROM users;", -1, &stmt, nullptr) == SQLITE_OK) {
            if (sqlite3_step(stmt) == SQLITE_ROW) {
                totalUsers = sqlite3_column_int(stmt, 0);
                activeUsers = sqlite3_column_int(stmt, 1);
                staffCount = sqlite3_column_int(stmt, 2);
            }
            sqlite3_finalize(stmt);
        }

        if (sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM departments;", -1, &stmt, nullptr) == SQLITE_OK) {
            if (sqlite3_step(stmt) == SQLITE_ROW) departmentsCount = sqlite3_column_int(stmt, 0);
            sqlite3_finalize(stmt);
        }

        if (sqlite3_prepare_v2(db, "SELECT COUNT(*) FROM locations;", -1, &stmt, nullptr) == SQLITE_OK) {
            if (sqlite3_step(stmt) == SQLITE_ROW) locationsCount = sqlite3_column_int(stmt, 0);
            sqlite3_finalize(stmt);
        }
        ...
        ss << "\"active_users\":" << activeUsers << ",";
        ss << "\"staff\":" << staffCount << ",";
        ss << "\"departments\":" << departmentsCount << ",";
        ss << "\"locations\":" << locationsCount << ",";
        ss << "\"system_mode\":\"Sole Administrator Controlled Directory\",";
        ss << "\"storage\":\"SQLite WAL (High Performance)\"";
        ss << "}";
        return ss.str();
    }
    ...
    std::string listDepartments() {
        sqlite3_stmt* stmt = nullptr;
        std::string sql = "SELECT id, name, code, manager, headcount FROM departments ORDER BY name ASC;";
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            const char* mgr = (const char*)sqlite3_column_text(stmt, 3);
            ss << "\"manager\":\"" << (mgr ? escapeJson(mgr) : "") << "\",";
    ...
    std::string listLocations() {
        sqlite3_stmt* stmt = nullptr;
        std::string sql = "SELECT id, name, city, country, address, timezone FROM locations ORDER BY name ASC;";
        if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) return "[]";
    ...
    std::string getActivityLogs() {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            ss << "\"description\":\"" << escapeJson((const char*)sqlite3_column_text(stmt, 4)) << "\",";
    ...
    void logActivity(const std::string& userId, const std::string& actorName, const std::string& action, const std::string& description) {
        std::string actId = "act_" + std::to_string(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count());
        sqlite3_stmt* stmt = nullptr;
        std::string sql = "INSERT INTO activity_logs (id, user_id, actor_name, action, description, created_at) VALUES (?, ?, ?, ?, ?, datetime('now'));";
        if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
            sqlite3_bind_text(stmt, 1, actId.c_str(), -1, SQLITE_TRANSIENT);
            if (!userId.empty()) sqlite3_bind_text(stmt, 2, userId.c_str(), -1, SQLITE_TRANSIENT);
    ...
    std::string createResetCode(const std::string& email, const std::string& code, int expiresInSeconds) {
        sqlite3_stmt* stmt = nullptr;
        std::string checkSql = "SELECT id, name, email FROM users WHERE lower(email) = lower(?);";
        if (sqlite3_prepare_v2(db, checkSql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
            return "{\"success\":false,\"error\":\"Database error checking account\"}";
        }
        sqlite3_bind_text(stmt, 1, email.c_str(), -1, SQLITE_STATIC);
        if (sqlite3_step(stmt) != SQLITE_ROW) {
            sqlite3_finalize(stmt);
        ...
        std::string userEmail = (const char*)sqlite3_column_text(stmt, 2);
        sqlite3_finalize(stmt);
        ...
        std::string insertSql = "INSERT OR REPLACE INTO password_resets (email, code, expires_at, created_at, attempts) VALUES (lower(?), ?, ?, datetime('now'), 0);";
        if (sqlite3_prepare_v2(db, insertSql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
            return "{\"success\":false,\"error\":\"Database error storing verification code\"}";
        }
        sqlite3_bind_text(stmt, 1, email.c_str(), -1, SQLITE_STATIC);
        ...
        logActivity(userId, userName, "Password Reset Code Generated", "Generated verification code for email: " + userEmail);

        std::ostringstream ss;
        ss << "{\"success\":true,\"user_id\":\"" << escapeJson(userId) << "\",\"name\":\"" << escapeJson(userName) << "\",\"email\":\"" << escapeJson(userEmail) << "\",\"expires_at\":" << expiresAt << "}";
        return ss.str();
    }
    ...
    std::string verifyResetCode(const std::string& email, const std::string& code) {
        ...
        if (sqlite3_step(stmt) != SQLITE_ROW) {
            sqlite3_finalize(stmt);
            return "{\"success\":false,\"error\":\"No active reset code found for this email. Please request a new code.\"}";
        }

        std::string storedCode = (const char*)sqlite3_column_text(stmt, 0);
        long long expiresAt = sqlite3_column_int64(stmt, 1);
...
#if defined(_MSC_VER)
        return "Microsoft Visual C++ (MSVC) " + std::to_string(_MSC_VER);
#elif defined(__MINGW64__)
        return "MinGW-w64 GCC " + std::string(__VERSION__) + " (Windows x64 -O3)";
#elif defined(__MINGW32__)
        return "MinGW-w32 GCC " + std::string(__VERSION__) + " (Windows x86 -O3)";
#elif defined(__clang__) && PLATFORM_WINDOWS
        return "Clang/LLVM for Windows (" + std::string(__clang_version__) + ")";
#elif defined(__clang__)
        return "Clang " + std::string(__clang_version__);
#elif defined(__GNUC__)
        return "g++ " + std::string(__VERSION__) + " (x86_64-linux-gnu -O3)";
#else
        return "Standard C++17 Compiler";
#endif
    }

    static std::string getOperatingSystem() {
#if PLATFORM_WINDOWS
        return "Microsoft Windows (Win32/Win64 Native)";
#elif defined(__APPLE__)
        return "macOS Darwin";
#elif defined(__linux__)
        return "Linux (x86_64 POSIX)";
#else
    ...
    std::string getEngineStatus() {
        std::ostringstream ss;
        ss << "{";
        ss << "\"engine_version\":\"Proton C++ v2.5.0 Cross-Platform\",";
        ss << "\"platform_os\":\"" << escapeJson(getOperatingSystem()) << "\",";
        ss << "\"compiler\":\"" << escapeJson(getCompilerName()) << "\",";
    ...
    std::string runBenchmark(int iterations = 1000) {
        ...
        ss << "\"journal_mode\":\"WAL\",";
        ss << "\"storage_engine\":\"Proton C++ Core v2.5.0\"";
        ss << "}";
        return ss.str();
    }
};
...
static inline std::string readStdinAll() {
    std::string result;
    std::string line;
...
int main(int argc, char* argv[]) {
#if PLATFORM_WINDOWS
    // Configure Windows console to UTF-8 output and input so unicode/JSON is preserved
    ...
    ProtonCore::Database db(dbPath);
    if (!db.open()) {
        std::cout << "{\"error\":\"Failed to initialize database\"}" << std::endl;
        return 1;
    }
